...
def compress_pdf(compression: int, folder_path: str) -> dict:
    ...
    if len(files) == 1:
        ...
        with ZipFile(result_path, "w") as archive:
            for file in files:
                pdf_path = _ghost_compress(compression, file)
                archive.write(pdf_path, arcname=pdf_path.name)
    return {"result_path": str(result_path)}


def _ghost_compress(compression: int, file: Path) -> Path:
...
def assemble_chunks(params: dict):
    ...
    chunks = sorted(
        [file for file in chunks_path.iterdir() if file.name.startswith("chunk_")]
    )
...
@celery_app.task
def terminate_task(task_id: str, folder_path: str):
